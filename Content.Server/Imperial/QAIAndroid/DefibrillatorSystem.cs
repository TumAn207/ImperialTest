...
/// </summary>
public sealed class QAIAndroidDefibrillatorSystem : EntitySystem
{
    ...
    {
        SubscribeLocalEvent<QAIAndroidDefibrillatorComponent, AfterInteractEvent>(OnAfterInteract);
        SubscribeLocalEvent<QAIAndroidDefibrillatorComponent, QAIDefibrillatorZapDoAfterEvent>(OnDoAfter);
    }

    private void OnAfterInteract(EntityUid uid, QAIAndroidDefibrillatorComponent component, AfterInteractEvent args)
    {
        if (args.Handled || !HasComp<QAIAndroidComponent>(args.Target) || args.Target is not { } target)
            return;

        args.Handled = TryStartZap(uid, target, args.User, component);
    }

    private void OnDoAfter(EntityUid uid, QAIAndroidDefibrillatorComponent component, QAIDefibrillatorZapDoAfterEvent args)
    {
    ...
    public bool CanZap(EntityUid uid, EntityUid target, EntityUid? user = null, QAIAndroidDefibrillatorComponent? component = null, bool targetCanBeAlive = false)
    {
    ...
    public bool TryStartZap(EntityUid uid, EntityUid target, EntityUid user, QAIAndroidDefibrillatorComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return false;
        ...
        return _doAfter.TryStartDoAfter(new DoAfterArgs(EntityManager, user, component.DoAfterDuration, new QAIDefibrillatorZapDoAfterEvent(),
            uid, target, uid)
        {
            NeedHand = true,
    ...
    public void Zap(EntityUid uid, EntityUid target, EntityUid user, QAIAndroidDefibrillatorComponent? component = null)
    {
        if (!Resolve(uid, ref component))
            return;
        ...
        var ev = new QAIAndroidTargetDefibrillatedEvent(user, (uid, component));
